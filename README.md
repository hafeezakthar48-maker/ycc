...
当前实现是工作流 MVP，数据保存在本地 SQLite 凭证库中，用于验证凭证中心产品体验；已具备基础权限、审计日志、数据库持久化、过账/反过账状态流、轻量账套隔离、会计期间关闭/重开状态和基于已审核凭证的总账/明细账/科目余额表只读视图。过账状态用于标记凭证工作流，不生成正式不可变总账分录；正式记账引擎、完整多账套核算和永久附件存储放入后续阶段。

## 账簿读模型 MVP

当前账簿视图支持：
- 总账：按期间汇总已审核凭证的科目借方、贷方、余额方向和分录数。
- 明细账：按期间与科目编码列出已审核凭证分录。
- 科目余额表：按期间展示科目余额、借贷合计和平衡状态。
- 账套与期间：查看默认账套和跨境电商账套，按账套读取会计期间状态、凭证数量和过账数量，支持关闭与重开期间。

该能力只读取凭证中心 SQLite 数据，不执行正式过账、结账、反结账或完整多账套核算；期间关闭是工作流状态控制，用于阻止继续过账，不等同于正式财务结账。
